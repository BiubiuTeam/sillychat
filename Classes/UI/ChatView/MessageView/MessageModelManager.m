...
+ (BOOL)emojiModelWithMessage:(EMMessage *)message emojiName:(NSString**)emoji
{
    *emoji = nil;
    id<IEMMessageBody> messageBody = [message.messageBodies firstObject];
    NSDictionary *userInfo = [[EaseMob sharedInstance].chatManager loginInfo];
    ...
    if (messageBody.messageBodyType == eMessageBodyType_Command) {
        NSString *actionTxt = [(EMCommandMessageBody *)messageBody action];
        if([actionTxt isEqualToString:@"sillychat#emoji"]){
            
            NSDictionary* param = [message ext];
            *emoji = [param objectForKey:@"sillychat#emoji"];
            return YES;
        }
        return NO;
    }
