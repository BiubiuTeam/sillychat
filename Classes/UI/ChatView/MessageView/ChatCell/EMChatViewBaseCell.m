...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    _nameLabel.hidden = YES;// !messageModel.isChatGroup;
    
    UIImage *placeholderImage = nil;
    if(messageModel.isSender){
        if ([[EMAccountService shareInstance] getAccountGendar] == 0) {
            _gendarIsMale = YES;
        }else{
            _gendarIsMale = NO;
        }
    ...
    if (_gendarIsMale) {
        placeholderImage = LOAD_ICON_USE_POOL_CACHE(@"private/silly_male_buddy_normal.png");
    }else{
        placeholderImage = LOAD_ICON_USE_POOL_CACHE(@"private/silly_female_buddy_normal.png");
    }
    [_headImageView setImage:placeholderImage];
}
